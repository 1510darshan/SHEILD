...
function makePosts(ngo,cnt=2) {
  return Array.from({length:cnt},(_,i)=>{
    ...
    return { id:`p${_pid++}`, ngoName:ngo.Name, ngoDomain:ngo.Domain, text:tmpl.text, tags:tmpl.tags,
...
/* HERO */
.hero{background:linear-gradient(135deg,#0a3d62 0%,#1a6fa8 60%,#0d5fa0 100%);padding:60px 24px;text-align:center;position:relative;overflow:hidden;}
...
export default function App() {
  ...
  const NAV = (
    <nav className="nav" style={{display:"flex", justifyContent:"flex-end", paddingRight:"24px"}}>
      {/* <div className="nlogo" onClick={()=>{setDetail(null);setTab("discover");}}>
        <div className="nlogo-ic">🤝</div>NGOConnect India
      </div> */}
      {/* <div className="ntabs">
        {[["discover","🏠 Discover"],["feed","📰 Feed"],["search","🔍 Search"]].map(([k,l])=>(
          <button key={k} className={`nt${tab===k&&!detail?" on":""}`} onClick={()=>{setDetail(null);setTab(k);}}>{l}</button>
        ))}
      </div> */}
      <button className="ncta" onClick={()=>{setDetail(null);setTab("add");}}>+Register</button>
    </nav>
  );
